...
FW_CALIB_LARGE = os.path.normpath(
    os.path.join(
        BASE_DIR,
        "..",
        "Firmware",
        "Large",
        "calibrationFirmware",
        "calibrationFirmware.ino",
    )
)
FW_CALIB_SMALL = os.path.normpath(
    os.path.join(
        BASE_DIR,
        "..",
        "Firmware",
...
FW_BIPED_LARGE = os.path.normpath(
    os.path.join(
        BASE_DIR, "..", "Firmware", "Large", "bipedFirmware", "bipedFirmware.ino"
    )
)
FW_BIPED_SMALL = os.path.normpath(
    os.path.join(
        BASE_DIR, "..", "Firmware", "Small", "bipedFirmware", "bipedFirmware.ino"
    )
)
...
def scan_and_match():
    ...
    for port in ports:
        if "STLink" in port.description or "STMicroelectronics" in port.manufacturer:
            ...
            if joint_name:
                print(
                    f"Match found! This is the {act_data['motor_type']} actuator for: {joint_name}"
                )
...
def command_position_smooth(
    port, target_q, transition_time=2.0, hold_time=5.0, kp=20.0, kd=0.2
):
...
def master_calibration():
    ...
    command_position_smooth(
        port, target_q=zero_offset, transition_time=2.0, hold_time=5.0
    )
